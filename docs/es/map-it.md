    ...
    <div id="sidebar-inner">
      ...
      <div class="mi-section">
        ...
        <div class="mi-section-body">
          ...
          <div id="id-editor-warning" class="mi-warning hidden">
            Solo "Herramientas" funcionan con el iD Editor
          </div>
        </div>
      </div>
      <!-- Overpass query buttons -->
      <div class="mi-section" id="section-overpass">
        <div class="mi-section-header static">Descarga capa</div>
      ...
      <div class="mi-section" id="id-url-section" style="display:none;">
        <div class="mi-section-body" id="id-url-container">
          <span class="mi-label">Copiar para iD → Datos del Mapa → Datos del Mapa Personalizados:</span>
      ...
      <div class="mi-section collapsed" id="section-hints">
        ...
        <div class="mi-section-body">
          ...
          <div id="osmose-panel" class="mi-hint-panel hidden">
            ...
            <div class="mi-meta-link">Advertencia: algunos países solo funcionan a nivel nacional.</div>
          </div>
          <div id="wikidata-panel" class="mi-hint-panel hidden">
            <label for="wikidataType">Tipo de datos:</label>
            <select id="wikidataType">
              <option value="All power-related infrastructure" selected>Todo</option>
              <option value="substations">Subestaciones</option>
              <option value="powerplants">Central electrica</option>
            </select>
            <div class="mi-meta-link">Solo nivel nacional.</div>
          </div>
          ...
          <div id="gem-panel"   class="mi-hint-panel hidden"><div class="mi-meta-link">Solo nivel nacional.</div></div>
          <div id="solar-panel" class="mi-hint-panel hidden"><div class="mi-meta-link">Solo nivel nacional.</div></div>
          <div id="wind-panel"  class="mi-hint-panel hidden"><div class="mi-meta-link">Solo nivel nacional.</div></div>
        </div>
      </div>
      <!-- Good First Lines -->
      <div class="mi-section collapsed">
        <button class="mi-section-header toggle" onclick="toggleSection(this)">
          <span>Buenas primeras lineas</span>
          <span class="mi-chevron">&#9660;</span>
        </button>
        <div class="mi-section-body">
          <p class="mi-body-text">Lineas que hemos encontrado! (🔶 países)</p>
          <a href="/good-first-lines/" class="mi-link-btn">Vea Buenas Primeras Líneas &#8594;</a>
        </div>
      </div>
      <!-- Map Legend -->
      <div class="mi-section">
        <button class="mi-icon-btn" onclick="openOverlay('legend')">
          <span class="mi-btn-label">&#128506;&#160; JOSM MapCSS Leyenda</span>
        </button>
      </div>
      <!-- JOSM Hotkeys -->
      <div class="mi-section">
        <button class="mi-icon-btn" onclick="openOverlay('hotkeys')">
          ...
          <span class="mi-btn-label">JOSM Atajos de teclado</span>
        </button>
      </div>
      <!-- Mapping Guidelines -->
      <div class="mi-section">
        <button class="mi-icon-btn" onclick="openOverlay('guidelines')">
          <span class="mi-btn-label">&#128214;&#160; Normas de mapeo</span>
        </button>
      </div>
      <!-- Good Practices -->
      <div class="mi-section">
        <button class="mi-icon-btn" onclick="openOverlay('practices')">
          <span class="mi-btn-label">&#9888;&#160; Buenas Prácticas</span>
        </button>
      </div>
      <!-- Community / Social -->
      <div class="mi-section collapsed">
        ...
        <div class="mi-section-body">
          ...
          <button class="mi-link-btn" onclick="openOverlay('calendar')">&#128197;&#160; Calendario comunitario</button>
        </div>
      </div>
...
const INFO_SLIDES = [
  {
    title: "Introducción",
    content: "<p>Bienvenido a nuestra plataforma interactiva para mapear la red eléctrica a través de <strong>OpenStreetMap</strong>!</p><p>Con tu ordenador, haz clic en un país o amplía el mapa y haz clic en una región para cargar los datos de la red en <strong>JOSM</strong> o <strong>iD</strong>.</p><p>Nuevo al mapeo de la red? Empieza con el <a href='/starter-kit/' target='_blank'>Kit de Inicio</a>. Usa el hashtag <strong>#MapYourGrid</strong>!</p>"
  },
  {
    title: "Cómo cargar los datos?",
    content: "<ol><li>Selecciona tu editor (JOSM o iD) en el panel.</li><li>Si JOSM, abre <a href='https://josm.openstreetmap.de/' target='_blank'>JOSM</a> y permite <em>Control Remoto</em> en Preferencias. Asegúrate de que tu adblocker esté desactivado.</li><li>Si iD, elija una capa de herramientas, y haz clic en un país/región.</li><li>Elija un típo de <strong>Overpass Query</strong> (en Descarga Capa).</li><li>Pulse un país— o haz zoom y pulse una región.</li><li>Use <strong>Herramientas</strong> para más capas de datos.</li><li>Países en naranja ya tienen una 'buena primera línea' que puedes mapear!</li></ol>"
  }
];
...
const PRACTICES_SLIDES = [
  {
    title: "Comunidades Locales & 'Good Practices'",
    content: `<p>Antes de empezar a trazar mapas, infórmate sobre las restricciones vigentes en el país correspondiente. En algunos países no está permitido trazar mapas de líneas de transmisión. Ponte en contacto con los usuarios locales buscando tu <a href='https://community.osm.be/' target='_blank'>comunidad local OSM</a> y <a href='https://wiki.openstreetmap.org/wiki/Power_networks#Local_projects' target='_blank'>proyectos locales</a>. Si no encuentras una comunidad local, por favor <a href='mailto:MapYourGrid@openenergytransition.org'>envié nos un correo</a> y podremos ayudarle.</p>
    <p>Siguiendo nuestros <a href='/mapping-good-practices/' target='_blank'>Good practices for mapping</a> (en inglés), protegemos la integridad de la plataforma OpenStreetMap, fomentamos la confianza con las comunidades y aprovechamos el potencial de los datos abiertos para lograr un futuro energético más resiliente y justo.</p><p> <strong>Por favor, NO copies ningún dato de las capas de sugerencias directamente en tu capa de datos de OpenStreetMap.</strong> </p><p> Cada dato debe introducirse y verificarse manualmente. Los metadatos también deben verificarse comparándolos con fuentes autorizadas compatibles o mediante personas sobre el terreno. Si no puedes verificar los datos utilizando imágenes de satélite o cualquier otra fuente compatible, no añadas esta información a partir de capas de sugerencias. Aunque al principio pueda parecer una tarea ardua, esto garantiza la alta calidad de OpenStreetMap.</p>`
  },
  {
    title: "Riesgo del doble mapeo",
    content: `<p>Ten en cuenta que solo has descargado los datos de la red de transporte del país, estado o provincia que has seleccionado. Esto incluye centrales eléctricas, generadores, subestaciones, torres de alta tensión y líneas de transporte. Otros elementos de OpenStreetMap, como las calles, no serán visibles. Por eso, <strong>Nunca utilices nuestras herramientas para mapear objetos que no estén cargados a través de nuestro Overpass</strong>, si no los demás colaboradores tendrán que eliminar los datos duplicados.</p>
    <p>Algunas líneas de transmisión transfronterizas seguirán siendo visibles más allá de los límites administrativos marcados en rosa. Sin embargo, para editarlas, tendrás que cargar ambos países. <strong>Nunca traces el mapa más allá de los límites administrativos marcados en rosa</strong>, ya que es muy probable que esto provoque que la infraestructura se trace dos veces.</p>`
  }
];
...
L.Control.LowVoltageToggle = L.Control.extend({
  onAdd() {
    ...
    btn.innerHTML = "220";
    btn.title = "Mostrar todas las líneas de 220+ kV";
...
L.Control.VoltageKey = L.Control.extend({
  ...
  onAdd(map) {
    ...
    panel.innerHTML = `
      <strong style="display:block;font-size:0.7rem;font-weight:700;color:#e8f5eb;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">Líneas</strong>
      ...
      <strong style="display:block;font-size:0.7rem;font-weight:700;color:#e8f5eb;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">Fronteras</strong>
      <div style="display:flex;flex-direction:column;gap:6px;">
        <div style="display:flex;align-items:center;gap:8px;font-size:0.75rem;color:#c8deca;">
          <span style="display:inline-block;width:22px;height:3px;border-radius:2px;background:#e07000;flex-shrink:0;"></span>País con buena primera línea
        </div>
      </div>`;
...
function buildHotkeysTable() {
  const rows = [
    ["S","Seleccionar objetos"], ["A","Modo estándar, dibujará líneas 'conectadas'"],
    ["Ctrl+C","Copiar"], ["Ctrl+V","Pegar"],
    ["Ctrl+F","Buscar (Modo Experto)"], ["Ctrl+Z","Deshacer"], ["Ctrl+Y","Rehacer"],
    ["Ctrl+Shift+C","Copiar coordenadas"], ["Ctrl+W","Alternar entre el modo vista y el modo de representación cartográfica"],
    ["Ctrl+H","Historico"], ["Ctrl+U","Recargar los objetos seleccionados actualmente del servidor"],
    ["Shift+V","Realizar comprobaciones de validación"], ["Tab","Panel"]
