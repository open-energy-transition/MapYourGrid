...
<div id="mapit-root">
  ...
  <aside id="mapit-sidebar">
    <div id="sidebar-inner">
      ...
      <!-- Overpass query buttons (populated by JS) -->
      ...
      <!-- iD URL box: shown between Overpass and Hints when iD mode is active -->
...
// Names that differ between the GeoJSON and a target API.
const OVERRIDES = {
  osmoseCountries: {
...
// Text content for the bottom-left info card carousel.
const INFO_SLIDES = [
  {
    title: "Introduction",
    content: "<p>Welcome to our interactive launchpad for contributing to power grid mapping via <strong>OpenStreetMap</strong>!</p><p>Click on a country or zoom in and click a region to load infrastructure data into <strong>JOSM</strong> or <strong>iD</strong>.</p><p>New to grid mapping? Start with the <a href='/starter-kit/' target='_blank'>Starter Kit</a>. Use the <strong>#MapYourGrid</strong> hashtag in your changesets!</p>"
  },
  {
...
const PRACTICES_SLIDES = [
  {
    ...
    content: "<p><strong>Before mapping, check whether your country has restrictions.</strong> In some countries, transmission line mapping is not permitted.</p><p>Find your <a href='https://community.osm.be/' target='_blank'>local OSM community</a> or <a href='https://wiki.openstreetmap.org/wiki/Power_networks#Local_projects' target='_blank'>local projects</a>. Can't find one? <a href='mailto:MapYourGrid@openenergytransition.org'>Email us</a>.</p><p>Follow our <a href='/mapping-good-practices/' target='_blank'>Good Practices guide</a>. <strong>Do NOT copy hint layer data directly into OSM.</strong> Every data point must be manually verified.</p>"
  },
  {
    title: "Risk of Double Mapping",
    content: "<p>You have only downloaded data for the <em>selected country or region</em>. Other OSM objects (streets, buildings) will not be visible.</p><p><strong>Never map objects outside your loaded extract</strong> — this creates duplicate data that other mappers must clean up.</p><p>Cross-border lines may appear beyond the pink boundary. To edit them, load both countries. <em>Never map beyond that boundary.</em></p>"
  }
];
...
// maxBounds restricts panning to exactly one world copy.
...
// noWrap: true stops the tile layer from repeating tiles outside [-180, 180].
...
// Fullscreen button (top-right, built with the Leaflet control API).
...
// Returns a Promise so the bootstrap can await it alongside fetchGFLCountries().
...
// Wires hover highlight and click handler onto a single country layer feature.
// GFL status is read from layer._isGFL (set later by applyGFLStyles).
function setupCountryLayer(layer) {
  ...
  layer.on("mouseout", () => {
    ...
    if (layer._isGFL) {
      layer.setStyle({ color: "#f97316", weight: 1.5, fillColor: "#fb923c", fillOpacity: 0.15 });
    } else {
...
async function fetchGFLCountries() {
  ...
  try {
    ...
    GFL_COUNTRY_NAMES = new Set(data.map(r => r.country.toLowerCase().trim()));
  } catch (err) {
...
function applyGFLStyles() {
  countriesLayer.eachLayer(layer => {
    ...
    if (GFL_COUNTRY_NAMES.has(name.toLowerCase())) {
      layer._isGFL = true;
      layer.setStyle({ color: "#e07000", weight: 1.5, fillColor: "#ff8c00", fillOpacity: 0.15 });
    }
  });
}

// Fetches regionsv2.geojson on first zoom into region level. No-op thereafter.
...
// Wires hover highlight and click handler onto a single region layer feature.
...
function buildToolButton(mode) {
  ...
  // Move the sub-option panel (if one exists for this mode) into this button's
  // group so it appears directly below its button, not at the bottom of the list.
  const PANEL_IDS = {
...
// Fetches and caches the version string for an overpass mode folder.
...
// ═══════════════════════════════════════════════════════════════════════════════
// INFO CARD  (bottom-left carousel)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// COUNTRY PANEL  (slides up from the bottom on map click)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// MODAL OVERLAY  (legend / hotkeys / guidelines / practices / calendar)
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildSuccessHtml() {
  ...
  return `<div class="popup-success">
    <p>Check <a href="https://josm.openstreetmap.de/" target="_blank">JOSM</a> — large countries may take 60 s or more.</p>
    <ol>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fetch GFL country list and country GeoJSON concurrently for speed,
  // then apply orange GFL highlighting once both are ready.
